...
// rosmium note: adapted from osmium-tool/src/version.cpp.in. Upstream fills
// in @PROJECT_VERSION@/@VERSION_FROM_GIT@ via CMake's configure_file() +
// `git describe`. rosmium vendors a fixed snapshot of osmium-tool rather
...
const char* get_osmium_long_version() noexcept {
    return "osmium version " OSMIUM_TOOL_VENDORED_VERSION " (vendored into rosmium)";
}
