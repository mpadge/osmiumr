...
[![Codecov test coverage](https://codecov.io/gh/mpadge/osmiumr/branch/main/graph/badge.svg)](https://codecov.io/gh/mpadge/osmiumr)
...
Grab a small real-world extract to work with -- the "ITS Leeds" test
file also used by [`osmextract`](https://docs.ropensci.org/osmextract/):
...
`osmium_extract()` trims a file down to a geographic boundary -- given
as a bounding box here (`c(left, bottom, right, top)`), or alternatively
a `.poly`/GeoJSON polygon file via `polygon`. The default
...
```{r extract-map, fig.alt = "Map of the geographic extract, showing points, lines and polygons around central ITS Leeds"}
