...
shelling out to an external `osmium` binary, osmiumr embeds
...
osmiumr needs is either a system library it detects at install time
(bz2, zlib, expat) or vendored source bundled with the package itself
(`libosmium`, `protozero`, `nlohmann-json`).

## Installation

You can install the development version of osmiumr from
...
Each `osmium-tool` command vendored into osmiumr has a corresponding
`osmium_*()` R function:
...
sources osmiumr wraps and vendors (see `LICENSE.txt`).
